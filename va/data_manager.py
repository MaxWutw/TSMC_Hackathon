...
class DataManagerService(DataManager):
    def get_artifact_prompt(self, ob):
        ...
        artifact_type = [item["type"] for item in inputs]
        artifact_name = [item["name"] for item in inputs]
        
        # Convert base64 strings to images
        artifact = [utils.convert_base64_to_image(item["base64"]) for item in inputs]
        artifact_type = [type(i) for i in (artifact)]
